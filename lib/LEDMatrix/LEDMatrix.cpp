...
{
    if (CONSTRUCTOR_DEBUGGING)
        DEBUG(String("baseMatrix ") + String(base_height) + "x" + String(base_width));
...
{
    if (CONSTRUCTOR_DEBUGGING)
        DEBUG(String("subMatrix ") + String(sub_height) + "x" + String(sub_width));
    ...
    // Dimensionscheck
    if (ASSERT_CHECK)
        ASSERT(row_offset + sub_height <= base_height);
    if (ASSERT_CHECK)
        ASSERT(column_offset + sub_width <= base_width);
}
...
{
    if (CONSTRUCTOR_DEBUGGING)
        DEBUG(String("vectorMatrix ") + String(sub_height) + "x" + String(sub_width));

    // Sicherstellen, das richtiger Typ an Konstruktor übergeben wurde
    if (ASSERT_CHECK)
        ASSERT(Type == MatrixType::ROW_VECTOR || Type == MatrixType::COLUMN_VECTOR);
...
{
    if (CONSTRUCTOR_DEBUGGING)
        DEBUG(String(" copy Matrix ") + String(sub_height) + "x" + String(sub_width));

    // Referenz-Check
    if (CONSTRUCTOR_DEBUGGING)
        DEBUG(String(" Referenz-Check: ") + (&this->complete_strip == &other_mat.complete_strip ? "true" : "false"));
    if (ASSERT_CHECK)
...
void LEDMatrix::operator=(LEDMatrix &other_mat)
{
    ...
    if (DEBUGGING)
        DEBUG(String("Dimesionscheck height ") + (this->matrix_height() == other_mat.matrix_height() ? "true" : "false"));
    if (DEBUGGING)
        DEBUG(String("Dimesionscheck width ") + (this->matrix_width() == other_mat.matrix_width() ? "true" : "false"));

    // Dimesionscheck
    if (ASSERT_CHECK)
        TEST(this->matrix_height() == other_mat.matrix_height());
    if (ASSERT_CHECK)
        TEST(this->matrix_width() == other_mat.matrix_width());
...
UINT_16 LEDMatrix::calc_pixel_position(const INT_16 row, const INT_16 column) const
{
    ...
    if (DEBUGGING)
        DEBUG(String("Matrix calc_pixel_position row check") + (row_index < (this->base_height) ? "true" : "false"));
    if (DEBUGGING)
        DEBUG(String("Matrix calc_pixel_position column check") + (column_index < (this->base_width) ? "true" : "false"));

    if (ASSERT_CHECK)
        TEST(row_index < (this->base_height));
    if (ASSERT_CHECK)
        TEST(column_index < (this->base_width));
    ...
    if (DEBUGGING)
        DEBUG(String("Check if LED-Position is valid") + String(led_position) + (led_position < (this->base_width) * (this->base_height) ? "true" : "false"));
    if (ASSERT_CHECK)
        TEST(led_position < (this->base_width) * (this->base_height));
...
void LEDMatrix::self_test()
{
    ...
    for (UINT_16 i = 0; i < this->sub_height; ++i)
    {
        for (UINT_16 j = 0; j < this->sub_width; ++j)
        {
            ...
            FastLED.show();
            if (DEBUGGING)
                DEBUG("LED " + String(i) + "x" + String(j) + " red");
            delay(delay_time_ms);
        }
    }
    ...
    for (UINT_16 i = 0; i < this->sub_height; ++i)
    {
        for (UINT_16 j = 0; j < this->sub_width; ++j)
        {
            ...
            FastLED.show();
            if (DEBUGGING)
                DEBUG("LED " + String(i) + "x" + String(j) + " green");
            delay(delay_time_ms);
        }
    }
    ...
    for (UINT_16 i = 0; i < this->sub_height; ++i)
    {
        for (UINT_16 j = 0; j < this->sub_width; ++j)
        {
            ...
            FastLED.show();
            if (DEBUGGING)
                DEBUG("LED " + String(i) + "x" + String(j) + " blue");
            delay(delay_time_ms);
        }
    }
