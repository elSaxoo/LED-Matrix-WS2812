...
// Die erste Dimension des Arrays stellt den Integer Wert des Zeichens dar.
    ...
    void print_char(LEDMatrix& mat, const char character, const CRGB color, const CRGB background)
    {
        ...
        if(DEBUGGING)    DEBUG(String("Printing char " + character));
        ...
        for(UINT_8 j = 0; j < char_width; ++j)
        {
            ...
            // Debug Ausgaben
            if(DEBUGGING)    {DEBUG("Setting up column " + String(j) + "\t"); Serial.println(column_bitmap, HEX);}
            ...
            for(UINT_8 i = 0; i < char_height; ++i)
            {
                ...
                // Debug Ausgaben
               if(DEBUGGING)     DEBUG("Setting up row " + String(i) + "\t" + "Testing: " + String(column_bitmap & (1<<i)) + "\t" + (column_bitmap & (1<<i) ? "Character" : "Backgroung"));
