...
const PROGMEM UINT_8 font[128][char_width] = {
...
void print_String(LEDMatrix &mat, const String text, const CRGB color, const int shift_horizontal, const CRGB background)
{
    ...
    UINT_8 k = 0;
    for (UINT_8 character = shift_horizontal / char_width; character < text_length && k < width; ++character)
    {
        // Zeichenreihen eines Zeichens durchlaufen bis zum Zeichenende.
        int i = 0;
        if (k == 0)
        {
            i = shift_horizontal % char_width;
        }

        while (i < char_width && k < width)
        {
            ...
            ++k;
            for (UINT_8 o = 0; o < char_height; ++o)
            {
                mat.pixel(o, k) = background;
            }
            ++i;
        }
...
void print_String(LEDMatrix &mat, const String text, const CHSV color, const int shift_horizontal, const CHSV background)
{
    print_String(mat, text, CRGB(color), shift_horizontal, CRGB(background));
}
